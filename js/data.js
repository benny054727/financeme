...
function migrate(){
  ...
  // תיקון רטרואקטיבי (גרסה 2 — אותה משפחת באג שוב, ראו התיקון המקביל להלוואות
  // למטה): מחיקת יעד/קרן ניקתה בעבר רק הפקדות של החודש הנוכחי, אז הפקדות
  // מחודשים קודמים נשארו יתומות והמשיכו להשפיע על היתרה המוצגת גם אחרי שהיעד
  // כבר לא קיים. עכשיו מנקים לפי אותו עיקרון כמו delGoal(): הפקדה ידנית יתומה
  // (בלי recurringId — קיימת רק בגלל היעד שנמחק) נמחקת לגמרי מכל חודש; הפקדה
  // מהוראת קבע יתומה (recurringId מוגדר — ההוראה ממשיכה לרוץ קדימה בלי קשר
  // ליעד) רק מתנקה מ-goalId ונשארת בהיסטוריה, כדי לא ליצור "חור" מלאכותי מול
  // מה שממשיך להירשם לאותה הוראת קבע בחודשים הבאים.
  DB.transactions.forEach(t=>{
    if(t.goalId&&t.recurringId&&!DB.goals.some(g=>g.id===t.goalId))t.goalId=null;
  });
  DB.transactions=DB.transactions.filter(t=>!(t.goalId&&!t.recurringId&&!DB.goals.some(g=>g.id===t.goalId)));
  // תיקון רטרואקטיבי: לפני התיקון ל-delLoan() (אותה משפחת באג בדיוק), מחיקת הלוואה
  // ניקתה רק את תשלום החודש הנוכחי, לא את כל התשלומים שכבר נוצרו לה (genLoanPayments
  // מייצר גם חודש-1 אחורה) — הם נשארו תנועות יתומות שממשיכות להשפיע על היתרה המוצגת
  // למרות שההלוואה כבר לא קיימת. מנקים כאן פעם אחת את כל מה שיתום (loanId שלא שייך
  // לאף הלוואה קיימת), לא רק החודש הנוכחי — בהתאמה לעיקרון החדש של delLoan().
  DB.transactions=DB.transactions.filter(t=>!(t.loanId&&!DB.loans.some(l=>l.id===t.loanId)));
  // תיקון רטרואקטיבי: לפני שעיגלנו ב-genLoanPayments(), תשלום הלוואה נשמר עם
  // הסכום הגולמי של נוסחת השפיצר (כמעט אף פעם לא עגול — למשל 616.7358833007362).
  // fmt() מעגל לתצוגה בכל מקום שמציג את הסכום, אבל שדה עריכה מספרי (openTx) מציג
  // את הערך הגולמי כמו שהוא. מעגלים כאן פעם אחת לשקל שלם, עקבי עם שאר האפליקציה.
  DB.transactions.forEach(t=>{if(t.loanId)t.amount=Math.round(t.amount);});
}
function save(){
